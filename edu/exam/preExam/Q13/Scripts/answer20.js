// Function to attach event listener to the solution button for question 20
function attachSolutionButtonListeners_question20(button) {
    button.addEventListener('click', function() {
        // Reset any previous styles
        document.querySelectorAll('input[name="question20"]').forEach(input => {
            input.parentElement.classList.remove('incorrect', 'highlight');
        });

        // Check the selected answer
        const selected = document.querySelector('input[name="question20"]:checked');
