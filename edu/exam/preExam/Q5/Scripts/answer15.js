...
function attachSolutionButtonListeners_question15(button) {
  button.addEventListener('click', function() {
      ...
      const correctAnswers = [
          "Azure SQL Database single database", // For serverless configuration
          "Azure SQL Managed instance",         // For highest compatibility
          "Azure SQL Database single database"  // For low-cost, low-maintenance database
      ];

      correctAnswers.forEach((answer, index) => {
          const dropZone = document.getElementById(`dropZone15-${index + 1}`);
      ...
      if (solutionInfoElement) {
          const allCorrect = correctAnswers.every((answer, index) => 
              document.getElementById(`dropZone15-${index + 1}`).textContent.trim() === answer
          );
