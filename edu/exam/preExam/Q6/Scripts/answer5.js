  ...
  button.addEventListener('click', function() {
    ...
    correctAnswers.forEach(answer => {
      if (selectedAnswers.includes(answer)) {
        document.querySelector(`input[name="question5_checkbox"][value="${answer}"]`).parentElement.classList.add('highlight');
      }
    });
    ...
    if (anyIncorrect) {
      selectedAnswers.forEach(answer => {
        if (!correctAnswers.includes(answer)) {
          document.querySelector(`input[name="question5_checkbox"][value="${answer}"]`).parentElement.classList.add('incorrect');
        }
      });
    } else {
