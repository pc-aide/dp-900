function attachSolutionButtonListeners_question7(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswer = document.querySelector('input[name="storageService7"]:checked');
    
    // Reset styles for all options
    document.querySelectorAll('input[name="storageService7"]').forEach(input => {
    ...
    if (selectedAnswer && selectedAnswer.value === correctAnswer) {
      ...
      document.getElementById('correctAnswers_question7').style.display = 'block';
