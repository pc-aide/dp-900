function attachSolutionButtonListeners_question3(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswer = document.querySelector('input[name="sqlType"]:checked');
    
    // Reset styles for all options
    document.querySelectorAll('input[name="sqlType"]').forEach(input => {
    ...
    if (selectedAnswer && selectedAnswer.value === correctAnswer) {
      ...
      document.getElementById('correctAnswers_question3').style.display = 'block';
