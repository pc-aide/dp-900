...
function attachSolutionButtonListeners_question12(button) {
  button.addEventListener('click', function() {
    ...
    document.querySelectorAll('input[name="question12_checkbox"]').forEach(input => {
      const label = input.parentElement;
      label.classList.remove('incorrect', 'highlight');
    });

    // Check the selected answers
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="question12_checkbox"]:checked')).map(input => input.value);
    ...
    correctAnswers.forEach(answer => {
      if (selectedAnswers.includes(answer)) {
        document.querySelector(`input[name="question12_checkbox"][value="${answer}"]`).parentElement.classList.add('highlight');
      }
    });
    ...
    if (anyIncorrect) {
      selectedAnswers.forEach(answer => {
        if (!correctAnswers.includes(answer)) {
          document.querySelector(`input[name="question12_checkbox"][value="${answer}"]`).parentElement.classList.add('incorrect');
        }
      });
    }
